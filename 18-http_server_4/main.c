...
static void usage_and_exit(const char *basename)
{
	...
	fprintf(stderr, "\t%s client <url>\n", basename);

	exit(1);
}

static void run_application_http(const char *basename, char **args, int n)
{
	pthread_t thread;
	...
	if (strcmp(args[0], "server") == 0) {
		...
		pthread_create(&thread, NULL, http_server, args);
	}
	else if (strcmp(args[0], "client") == 0) {
		if (n != 2)
			usage_and_exit(basename);

		pthread_create(&thread, NULL, http_client, args);
	}
	else {
		usage_and_exit(basename);
	}
...
int main(int argc, char **argv)
{
	...
	run_application_http((const char *)basename(argv[0]), argv+1, argc-1);
