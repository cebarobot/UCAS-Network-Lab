...
struct server_vars * get_sv_of_sock(struct tcp_sock * sk) {
	for (int i = 0; i < svars_cnt; i++) {
		if (svars[i].sk == sk) {
			return &svars[i];
		}
	}
	struct server_vars * res = &svars[svars_cnt];
	svars_cnt += 1;
	return res;
}
...
int handle_connection(struct tcp_sock * sockid, struct server_vars *sv)
{
	...
	if (sv->request_len <= 0) {
		fprintf(stderr, "Socket %p: Failed to parse HTTP request header.\n"
...
struct tcp_sock * accept_connection(struct tcp_sock * listener)
{
	...
	struct tcp_sock * c = tcp_sock_accept(listener);
	// struct server_vars *sv = &svars[c];
	struct server_vars *sv = get_sv_of_sock(c);
	clean_server_variable(sv);
...
struct tcp_sock * create_listening_socket()
{
	...
	int ret = tcp_sock_bind(listener, &saddr);
	if (ret < 0) {
	...
	ret = tcp_sock_listen(listener, backlog);
	if (ret < 0) {
...
{
	struct tcp_sock * fd = *(struct tcp_sock * *)arg;
	// struct server_vars *sv = &svars[fd];
	struct server_vars *sv = get_sv_of_sock(fd);
...
void run_server()
{
	...
	while (TRUE) {
		struct tcp_sock * ret = accept_connection(listener);
		if (ret < 0)
			break;

		struct tcp_sock * cfd = ret;
		pthread_t pid;
